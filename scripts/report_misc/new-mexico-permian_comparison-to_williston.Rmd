...
perm_vs_others <- ggplot(split_in_3, aes(PublishDate, n, color = l)) +
 ...
 blg_theme_default(text_size = b_size)
...
perm_map <- perm %>%
 ...
 theme(plot.background = element_rect("white", "white", size = 0),
       legend.position = c(0.52, 0.23), # 0.22, 0.25
       legend.direction = "horizontal",
       # legend.text = element_text(color = "white"),
       # legend.title = element_text(color = "white"),
       NULL)
 # blg_theme_default(text_size = b_size - 4)
...
m <- image_read(perm_map_f) %>%
 image_trim() %>%
 # image_resize("600x") %>%
 # image_border(bg_col, "50x0")
 image_resize("700x") %>%
 image_crop("627x573+40+95") #%>% # 660x570
 # image_border(bg_col, "20x0")  #20x0

top_border <- image_blank(627, 85, bg_col) # 90
bot_border <- image_blank(627, 42, bg_col) # 40

side1_border <- image_blank(60, 700, bg_col) # 40
side2_border <- image_blank(13, 700, bg_col) # 20
...
top %>%
 ...
 # Add the title
 image_join(
  image_blank(image_info(.)$width, 100, bg_col) %>%
   image_annotate("A Look At The Permian"
                  , font = "Lato", size = 65, color = f_col
 ...
 # Add caption
 image_join(
  image_blank(image_info(.)$width, 50, bg_col) %>%
...
#  mutate(csum = cumsum(n > 0)) %>%
#  ungroup() %>%
#  ggplot(aes(Date, csum, color = basin)) +
#  geom_line() +
#  blg_theme_default() +
...
#  ggplot(aes(Date, csum, color = basin)) +
#  geom_line() +
#  scale_x_date(breaks = as.Date(c("2018-01-01", "2018-04-01", "2018-07-01", "2018-10-01")),
#               labels = c("Jan", "Apr", "Jul", "Oct"),
#               limits = as.Date(c("2018-01-01", "2018-12-31"))) +
#  scale_color_manual(values = pals::cols25(15)) +
...
p_cum_pos1 <- cum_pos %>%
 ...
 geom_line() +
 geom_text(aes(label = year), l_cum_pos_lab1, nudge_y = 1.5) +
 scale_x_continuous(NULL,
                    breaks = c(1, 14, 27, 40),
                    labels = c("Jan", "Apr", "Jul", "Oct")) +
 scale_y_continuous(NULL) +
 scale_color_manual(values = c("plum3", "plum3", "plum3", "plum4"),
                    guide = FALSE) +
 blg_theme_default() +
 theme(panel.grid.major.x = element_blank())

ggsave(f_cum_pos1 <- tempfile("p_cum_pos1", tmp_dir, ".png"),
       p_cum_pos1, width = 5, height = 4, dpi = 600)
...
p_cum_pos2 <- cum_pos %>%
 ...
 geom_line() +
 geom_text(aes(label = year), l_cum_pos_lab2, nudge_y = 30) +
 scale_x_continuous(NULL,
                    breaks = c(1, 14, 27, 40),
                    labels = c("Jan", "Apr", "Jul", "Oct")) +
 scale_y_continuous(NULL) +
...
ggsave(f_cum_pos2 <- tempfile("p_cum_pos2", tmp_dir, ".png"),
       p_cum_pos2, width = 5, height = 4, dpi = 600)
...
f_cum_pos1 %>%
 image_read() %>%
 image_join(
  image_blank(image_info(.)$width, hh, bg_col) %>%
   image_annotate("Cumulative Weeks of Positive Growth"
                  , font = "Lato", size = hf, color = f_col
 ...
 # sub-title
 image_join(
  image_blank(image_info(.)$width, hh * 1.6, bg_col) %>%
 ...
 # main title
 image_join(
  image_blank(image_info(.)$width, hh * 1.3, bg_col) %>%
   image_annotate("Rig Count Changes In The Permian"
                  , font = "Lato", size = hf * 1.4, color = f_col
                  , location = geometry_point(gx, 5))
  , .
 ) %>%
 image_append(TRUE) %>%
 ...
 # caption
 image_join(
  image_blank(image_info(.)$width, hh * 0.8, bg_col) %>%
   image_annotate("Source: Baker Hughes Rig Count"
