...
`%not_in%` <- compose(`!`, `%in%`)
qry_cat <- 2477496  # All states, monthly bpd
...
# Load meta data
load("~/R/oil/data/prod/cat_2477496_meta.RData")

# Extract series to 
meta_series <- cat_2477496_meta %>%
 pull("series_id") %>%
 set_names(nm = cat_2477496_meta %>% pull("state"))
...
# Extract
cat_2477496_meta2 <- res_series %>%
 map(~.x[, names(.x) %not_in% "data"]) %>%
 bind_rows() %>%
 as_tibble()

# Combine
exclude <- intersect(names(cat_2477496_meta), names(cat_2477496_meta2))
exclude <- exclude[exclude != "series_id"]

cat_2477496_meta <- cat_2477496_meta %>%
 inner_join(cat_2477496_meta2 %>%
             select(not_one_of(exclude)), by = "series_id")

# Save
save(cat_2477496_meta, file = "~/R/oil/data/prod/cat_2477496_meta.RData")
...
# Rename
us_crude_prod_api <- res_data

# Save
save(us_crude_prod, file = "~/R/oil/data/prod/us_crude_prod_api.RData")
